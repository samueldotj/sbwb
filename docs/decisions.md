...
| D-31 | Region OCR merges as candidates only: a reading that differs from the current span becomes an open proposal (region OCR keeps the word confidence; the second engine, `ocrs`, is line-level and stored unscored), words that overlap no span are inserted as new spans in reading order, and nothing is applied automatically even above the auto-apply threshold (OCR-02). Structural issues (missing text, clipping, uncertain order) live in the same index with a page box instead of a span; clipping is judged on recognised words, not region boxes. | OCR-02, LAY-03, REV-01 | 2026-09-12 |
| D-32 | AI payloads are the saved words of the selected pages with per-word ids and the page numbers, framed as data in the prompt; replies are accepted only when the `before` text equals the sent word, the replacement is a word-level change, and the span revision is unchanged at merge time. Suggestions are proposals with source `ai:<provider>/<model>` and an unscored candidate; the review threshold never hides or applies them. Keys live in the process (session) or the OS credential store, never in the project. `SBWB_AI_<PROVIDER>_BASE` points the adapters at a stand-in server for tests. | AI-01..03 | 2026-09-12 |
| D-33 | The book queue runs one pipeline at a time and is mutually exclusive with interactive processing; the queue file lives in the app data folder and a run interrupted by closing the app is marked interrupted, not resumed. Removing a queued book never deletes its project. | PRJ-06 | 2026-09-12 |
