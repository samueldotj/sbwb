# Third-party notices

SBWB is MIT licensed. It links or bundles the following third-party software. Full license texts are available from each project; the crate texts are in the Cargo registry sources (`cargo license` lists them).

## Rust crates

| Crate | Version | License | Authors |
| --- | --- | --- | --- |
| ab_glyph | 0.2.32 | Apache-2.0 | Alex Butler <alexheretic@gmail.com> |
| ab_glyph_rasterizer | 0.1.10 | Apache-2.0 | Alex Butler <alexheretic@gmail.com> |
| adler2 | 2.0.1 | 0BSD OR Apache-2.0 OR MIT | Jonas Schievink <jonasschievink@gmail.com>, oyvindln <oyvindln@users.noreply.git |
| aes | 0.9.3 | Apache-2.0 OR MIT | RustCrypto Developers |
| aho-corasick | 1.1.5 | MIT OR Unlicense | Andrew Gallant <jamslam@gmail.com> |
| aligned | 0.4.3 | Apache-2.0 OR MIT |  |
| aligned-vec | 0.6.4 | MIT | sarah <> |
| alloc-no-stdlib | 2.0.4 | BSD-3-Clause | Daniel Reiter Horn <danielrh@dropbox.com> |
| alloc-stdlib | 0.2.4 | BSD-3-Clause | Daniel Reiter Horn <danielrh@dropbox.com> |
| android_system_properties | 0.1.6 | Apache-2.0 OR MIT | Nicolas Silva <nical@fastmail.com> |
| anyhow | 1.0.104 | Apache-2.0 OR MIT | David Tolnay <dtolnay@gmail.com> |
| apple-native-keyring-store | 1.0.2 | Apache-2.0 OR MIT | Daniel Brotsky <dev@brotsky.com> |
| approx | 0.5.1 | Apache-2.0 | Brendan Zabarauskas <bjzaba@yahoo.com.au> |
| arbitrary | 1.4.2 | Apache-2.0 OR MIT | The Rust-Fuzz Project Developers, Nick Fitzgerald <fitzgen@gmail.com>, Manish Go |
| arg_enum_proc_macro | 0.3.4 | MIT | Luca Barbato <lu_zero@gentoo.org> |
| arrayvec | 0.7.8 | Apache-2.0 OR MIT | bluss |
| as-slice | 0.2.1 | Apache-2.0 OR MIT | Jorge Aparicio <jorge@japaric.io>, Emil Fresk <emil.fresk@gmail.com> |
| async-broadcast | 0.7.2 | Apache-2.0 OR MIT | Stjepan Glavina <stjepang@gmail.com>, Yoshua Wuyts <yoshuawuyts@gmail.com>, Zees |
| async-channel | 2.5.0 | Apache-2.0 OR MIT | Stjepan Glavina <stjepang@gmail.com> |
| async-executor | 1.14.0 | Apache-2.0 OR MIT | Stjepan Glavina <stjepang@gmail.com>, John Nunley <dev@notgull.net> |
| async-io | 2.6.0 | Apache-2.0 OR MIT | Stjepan Glavina <stjepang@gmail.com> |
| async-lock | 3.4.2 | Apache-2.0 OR MIT | Stjepan Glavina <stjepang@gmail.com> |
| async-process | 2.5.0 | Apache-2.0 OR MIT | Stjepan Glavina <stjepang@gmail.com> |
| async-recursion | 1.1.1 | Apache-2.0 OR MIT | Robert Usher <266585+dcchut@users.noreply.github.com> |
| async-signal | 0.2.14 | Apache-2.0 OR MIT | John Nunley <dev@notgull.net> |
| async-task | 4.7.1 | Apache-2.0 OR MIT | Stjepan Glavina <stjepang@gmail.com> |
| async-trait | 0.1.92 | Apache-2.0 OR MIT | David Tolnay <dtolnay@gmail.com> |
| atk | 0.18.2 | MIT | The gtk-rs Project Developers |
| atk-sys | 0.18.2 | MIT | The gtk-rs Project Developers |
| atomic-waker | 1.1.2 | Apache-2.0 OR MIT | Stjepan Glavina <stjepang@gmail.com>, Contributors to futures-rs |
| autocfg | 1.5.1 | Apache-2.0 OR MIT | Josh Stone <cuviper@gmail.com> |
| av-scenechange | 0.14.1 | MIT | Josh Holmer <jholmer.in@gmail.com> |
| av1-grain | 0.2.5 | BSD-2-Clause |  |
| avif-serialize | 0.8.9 | BSD-3-Clause | Kornel Lesiński <kornel@geekhood.net> |
| aws-lc-rs | 1.18.1 | (Apache-2.0 OR ISC) AND ISC | AWS-LibCrypto |
| aws-lc-sys | 0.45.0 | (Apache-2.0 OR ISC OR MIT) AND (Apache-2.0 OR ISC OR MIT-0) AND (Apache-2.0 OR ISC) AND Apache-2.0 AND BSD-3-Clause AND ISC AND MIT | AWS-LC |
| base64 | 0.21.7 | Apache-2.0 OR MIT | Alice Maz <alice@alicemaz.com>, Marshall Pierce <marshall@mpierce.org> |
| base64 | 0.22.1 | Apache-2.0 OR MIT | Marshall Pierce <marshall@mpierce.org> |
| base64 | 0.23.1 | Apache-2.0 OR MIT | Marshall Pierce <marshall@mpierce.org> |
| bindgen | 0.64.0 | BSD-3-Clause | Jyun-Yan You <jyyou.tw@gmail.com>, Emilio Cobos Álvarez <emilio@crisal.io>, Nick |
| bit-set | 0.8.0 | Apache-2.0 OR MIT | Alexis Beingessner <a.beingessner@gmail.com> |
| bit-vec | 0.8.0 | Apache-2.0 OR MIT | Alexis Beingessner <a.beingessner@gmail.com> |
| bit_field | 0.10.3 | Apache-2.0 OR MIT | Philipp Oppermann <dev@phil-opp.com> |
| bitflags | 1.3.2 | Apache-2.0 OR MIT | The Rust Project Developers |
| bitflags | 2.13.2 | Apache-2.0 OR MIT | The Rust Project Developers |
| bitstream-io | 4.10.0 | Apache-2.0 OR MIT | Brian Langenberger <bjl@usa.net> |
| blake3 | 1.8.7 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR CC0-1.0 | Jack O'Connor <oconnor663@gmail.com>, Samuel Neves |
| block-buffer | 0.10.4 | Apache-2.0 OR MIT | RustCrypto Developers |
| block-buffer | 0.12.1 | Apache-2.0 OR MIT | RustCrypto Developers |
| block-padding | 0.4.2 | Apache-2.0 OR MIT | RustCrypto Developers |
| block2 | 0.6.2 | MIT | Mads Marquart <mads@marquart.dk> |
| blocking | 1.7.0 | Apache-2.0 OR MIT |  |
| brotli | 8.0.4 | BSD-3-Clause AND MIT | Daniel Reiter Horn <danielrh@dropbox.com>, The Brotli Authors |
| brotli-decompressor | 5.0.3 | BSD-3-Clause OR MIT | Daniel Reiter Horn <danielrh@dropbox.com>, The Brotli Authors |
| bs58 | 0.5.1 | Apache-2.0 OR MIT |  |
| built | 0.8.1 | MIT | Lukas Lueg <lukas.lueg@gmail.com> |
| bumpalo | 3.20.3 | Apache-2.0 OR MIT | Nick Fitzgerald <fitzgen@gmail.com> |
| bytemuck | 1.25.2 | Apache-2.0 OR MIT OR Zlib | Lokathor <zefria@gmail.com> |
| byteorder | 1.5.0 | MIT OR Unlicense | Andrew Gallant <jamslam@gmail.com> |
| byteorder-lite | 0.1.0 | MIT OR Unlicense |  |
| bytes | 1.12.1 | MIT | Carl Lerche <me@carllerche.com>, Sean McArthur <sean@seanmonstar.com> |
| cairo-rs | 0.18.5 | MIT | The gtk-rs Project Developers |
| cairo-sys-rs | 0.18.2 | MIT | The gtk-rs Project Developers |
| camino | 1.2.5 | Apache-2.0 OR MIT | Without Boats <saoirse@without.boats>, Ashley Williams <ashley666ashley@gmail.co |
| cargo-platform | 0.1.9 | Apache-2.0 OR MIT |  |
| cargo_metadata | 0.19.2 | MIT | Oliver Schneider <git-spam-no-reply9815368754983@oli-obk.de> |
| cargo_toml | 0.22.3 | Apache-2.0 OR MIT | Kornel <kornel@geekhood.net> |
| cbc | 0.2.1 | Apache-2.0 OR MIT | RustCrypto Developers |
| cc | 1.4.5 | Apache-2.0 OR MIT |  |
| cesu8 | 1.1.0 | Apache-2.0 OR MIT | Eric Kidd <git@randomhacks.net> |
| cexpr | 0.6.0 | Apache-2.0 OR MIT | Jethro Beekman <jethro@jbeekman.nl> |
| cfb | 0.7.3 | MIT | Matthew D. Steele <mdsteele@alum.mit.edu> |
| cfg-expr | 0.15.8 | Apache-2.0 OR MIT | Embark <opensource@embark-studios.com>, Jake Shadle <jake.shadle@embark-studios. |
| cfg-if | 1.0.4 | Apache-2.0 OR MIT | Alex Crichton <alex@alexcrichton.com> |
| cfg_aliases | 0.2.2 | MIT | Zicklag <zicklag@katharostech.com> |
| chacha20 | 0.10.2 | Apache-2.0 OR MIT | RustCrypto Developers |
| chrono | 0.4.45 | Apache-2.0 OR MIT |  |
| cipher | 0.5.2 | Apache-2.0 OR MIT | RustCrypto Developers |
| clang-sys | 1.9.1 | Apache-2.0 | Kyle Mayes <kyle@mayeses.com> |
| cmake | 0.1.58 | Apache-2.0 OR MIT | Alex Crichton <alex@alexcrichton.com> |
| cmov | 0.5.4 | Apache-2.0 OR MIT | RustCrypto Developers |
| color_quant | 1.1.0 | MIT | nwin <nwin@users.noreply.github.com> |
| combine | 4.6.8 | MIT | Markus Westerlind <marwes91@gmail.com> |
| concurrent-queue | 2.5.0 | Apache-2.0 OR MIT | Stjepan Glavina <stjepang@gmail.com>, Taiki Endo <te316e89@gmail.com>, John Nunl |
| console_error_panic_hook | 0.1.7 | Apache-2.0 OR MIT | Nick Fitzgerald <fitzgen@gmail.com> |
| console_log | 1.1.0 | Apache-2.0 OR MIT | Matthew Nicholson <matt@matt-land.com> |
| const-oid | 0.10.2 | Apache-2.0 OR MIT | RustCrypto Developers |
| constant_time_eq | 0.4.2 | Apache-2.0 OR CC0-1.0 OR MIT-0 | Cesar Eduardo Barros <cesarb@cesarb.eti.br> |
| cookie | 0.18.2 | Apache-2.0 OR MIT | Sergio Benitez <sb@sergio.bz>, Alex Crichton <alex@alexcrichton.com> |
| core-foundation | 0.10.1 | Apache-2.0 OR MIT | The Servo Project Developers |
| core-foundation-sys | 0.8.7 | Apache-2.0 OR MIT | The Servo Project Developers |
| core-graphics | 0.25.0 | Apache-2.0 OR MIT | The Servo Project Developers |
| core-graphics-types | 0.2.0 | Apache-2.0 OR MIT | The Servo Project Developers |
| core_detect | 1.0.0 | Apache-2.0 OR MIT | Thom Chiovoloni <chiovolonit@gmail.com> |
| cpubits | 0.1.1 | Apache-2.0 OR MIT | RustCrypto Developers |
| cpufeatures | 0.2.17 | Apache-2.0 OR MIT | RustCrypto Developers |
| cpufeatures | 0.3.1 | Apache-2.0 OR MIT | RustCrypto Developers |
| crc32fast | 1.5.1 | Apache-2.0 OR MIT | Sam Rijs <srijs@airpost.net>, Alex Crichton <alex@alexcrichton.com> |
| crossbeam-channel | 0.5.17 | Apache-2.0 OR MIT |  |
| crossbeam-deque | 0.8.8 | Apache-2.0 OR MIT |  |
| crossbeam-epoch | 0.9.21 | Apache-2.0 OR MIT |  |
| crossbeam-utils | 0.8.23 | Apache-2.0 OR MIT |  |
| crunchy | 0.2.4 | MIT | Eira Fransham <jackefransham@gmail.com> |
| crypto-common | 0.1.7 | Apache-2.0 OR MIT | RustCrypto Developers |
| crypto-common | 0.2.2 | Apache-2.0 OR MIT | RustCrypto Developers |
| cssparser | 0.36.0 | MPL-2.0 | Simon Sapin <simon.sapin@exyr.org> |
| cssparser-macros | 0.6.1 | MPL-2.0 | Simon Sapin <simon.sapin@exyr.org> |
| ctor | 0.8.0 | Apache-2.0 OR MIT | Matt Mastracci <matthew@mastracci.com> |
| ctor-proc-macro | 0.0.7 | Apache-2.0 OR MIT | Matt Mastracci <matthew@mastracci.com> |
| ctutils | 0.4.2 | Apache-2.0 OR MIT | RustCrypto Developers |
| darling | 0.24.1 | MIT | Ted Driggs <ted.driggs@outlook.com> |
| darling_core | 0.24.1 | MIT | Ted Driggs <ted.driggs@outlook.com> |
| darling_macro | 0.24.1 | MIT | Ted Driggs <ted.driggs@outlook.com> |
| dbus | 0.9.12 | Apache-2.0 OR MIT | David Henningsson <diwic@ubuntu.com> |
| defmt | 1.1.1 | Apache-2.0 OR MIT | The Knurling-rs developers |
| defmt-macros | 1.1.1 | Apache-2.0 OR MIT | The Knurling-rs developers |
| defmt-parser | 1.0.0 | Apache-2.0 OR MIT | The Knurling-rs developers |
| deranged | 0.5.8 | Apache-2.0 OR MIT | Jacob Pratt <jacob@jhpratt.dev> |
| derive_more | 2.1.1 | MIT | Jelte Fennema <github-tech@jeltef.nl> |
| derive_more-impl | 2.1.1 | MIT | Jelte Fennema <github-tech@jeltef.nl> |
| digest | 0.10.7 | Apache-2.0 OR MIT | RustCrypto Developers |
| digest | 0.11.3 | Apache-2.0 OR MIT | RustCrypto Developers |
| dirs | 6.0.0 | Apache-2.0 OR MIT | Simon Ochsenreither <simon@ochsenreither.de> |
| dirs-sys | 0.5.0 | Apache-2.0 OR MIT | Simon Ochsenreither <simon@ochsenreither.de> |
| dispatch2 | 0.3.1 | Apache-2.0 OR MIT OR Zlib | Mads Marquart <mads@marquart.dk>, Mary <mary@mary.zone> |
| displaydoc | 0.2.7 | Apache-2.0 OR MIT | Jane Lusby <jlusby@yaah.dev> |
| dlopen2 | 0.8.2 | MIT | Szymon Wieloch <szymon.wieloch@gmail.com>, Ahmed Masud <ahmed.masud@saf.ai>, Ope |
| dlopen2_derive | 0.4.3 | MIT | Szymon Wieloch <szymon.wieloch@gmail.com>, OpenByte <development.openbyte@gmail. |
| document-features | 0.2.12 | Apache-2.0 OR MIT | Slint Developers <info@slint.dev> |
| docx-rs | 0.4.22 | MIT | bokuweb <bokuweb12@gmail.com> |
| dom_query | 0.27.0 | MIT | niklak <morgenpurple@gmail.com>, importcjj <importcjj@gmail.com> |
| dpi | 0.1.2 | Apache-2.0 AND MIT |  |
| dtoa | 1.0.11 | Apache-2.0 OR MIT | David Tolnay <dtolnay@gmail.com> |
| dtoa-short | 0.3.5 | MPL-2.0 | Xidorn Quan <me@upsuper.org> |
| dtor | 0.3.0 | Apache-2.0 OR MIT | Matt Mastracci <matthew@mastracci.com> |
| dtor-proc-macro | 0.0.6 | Apache-2.0 OR MIT | Matt Mastracci <matthew@mastracci.com> |
| dunce | 1.0.5 | Apache-2.0 OR CC0-1.0 OR MIT-0 | Kornel <kornel@geekhood.net> |
| dyn-clone | 1.0.20 | Apache-2.0 OR MIT | David Tolnay <dtolnay@gmail.com> |
| ecb | 0.2.1 | Apache-2.0 OR MIT | RustCrypto Developers |
| either | 1.18.0 | Apache-2.0 OR MIT |  |
| embed-resource | 3.0.11 | MIT | наб <nabijaczleweli@nabijaczleweli.xyz>, Cat Plus Plus <piotrlegnica@piotrl.pl>, |
| embed_plist | 1.2.2 | Apache-2.0 OR MIT | Nikolai Vazquez <hello@nikolaivazquez.com> |
| encoding_rs | 0.8.41 | (Apache-2.0 OR MIT) AND BSD-3-Clause | Henri Sivonen <hsivonen@hsivonen.fi> |
| endi | 1.1.1 | MIT | Zeeshan Ali Khan <zeenix@gmail.com> |
| enumflags2 | 0.7.12 | Apache-2.0 OR MIT | maik klein <maikklein@googlemail.com>, Maja Kądziołka <maya@compilercrim.es> |
| enumflags2_derive | 0.7.12 | Apache-2.0 OR MIT | maik klein <maikklein@googlemail.com>, Maja Kądziołka <maya@compilercrim.es> |
| equator | 0.4.2 | MIT | sarah <> |
| equator-macro | 0.4.2 | MIT | sarah <> |
| equivalent | 1.0.2 | Apache-2.0 OR MIT |  |
| erased-serde | 0.4.10 | Apache-2.0 OR MIT | David Tolnay <dtolnay@gmail.com> |
| errno | 0.3.14 | Apache-2.0 OR MIT | Chris Wong <lambda.fairy@gmail.com>, Dan Gohman <dev@sunfishcode.online> |
| event-listener | 5.4.2 | Apache-2.0 OR MIT | Stjepan Glavina <stjepang@gmail.com>, John Nunley <dev@notgull.net> |
| event-listener-strategy | 0.5.4 | Apache-2.0 OR MIT | John Nunley <dev@notgull.net> |
| exr | 1.74.2 | BSD-3-Clause | johannesvollmer <contact@johannesvollmer.com> |
| fallible-iterator | 0.3.0 | Apache-2.0 OR MIT | Steven Fackler <sfackler@gmail.com> |
| fallible-streaming-iterator | 0.1.9 | Apache-2.0 OR MIT | Steven Fackler <sfackler@gmail.com> |
| fast_image_resize | 6.1.0 | Apache-2.0 OR MIT | Kirill Kuzminykh <cykooz@gmail.com> |
| fastrand | 2.5.0 | Apache-2.0 OR MIT | Stjepan Glavina <stjepang@gmail.com> |
| fax | 0.2.7 | MIT | Sebastian K <s3bk@protonmail.com> |
| fdeflate | 0.3.7 | Apache-2.0 OR MIT | The image-rs Developers |
| field-offset | 0.3.6 | Apache-2.0 OR MIT | Diggory Blake <diggsey@googlemail.com> |
| find-msvc-tools | 0.1.12 | Apache-2.0 OR MIT |  |
| flatbuffers | 24.12.23 | Apache-2.0 | Robert Winslow <hello@rwinslow.com>, FlatBuffers Maintainers |
| flate2 | 1.1.10 | Apache-2.0 OR MIT | Alex Crichton <alex@alexcrichton.com>, Josh Triplett <josh@joshtriplett.org> |
| fnv | 1.0.7 | Apache-2.0 OR MIT | Alex Crichton <alex@alexcrichton.com> |
| foldhash | 0.2.0 | Zlib | Orson Peters <orsonpeters@gmail.com> |
| foreign-types | 0.5.0 | Apache-2.0 OR MIT | Steven Fackler <sfackler@gmail.com> |
| foreign-types-macros | 0.2.4 | Apache-2.0 OR MIT | Steven Fackler <sfackler@gmail.com> |
| foreign-types-shared | 0.3.1 | Apache-2.0 OR MIT | Steven Fackler <sfackler@gmail.com> |
| form_urlencoded | 1.2.2 | Apache-2.0 OR MIT | The rust-url developers |
| fs_extra | 1.3.0 | MIT | Denis Kurilenko <webdesus@gmail.com> |
| futures-channel | 0.3.34 | Apache-2.0 OR MIT |  |
| futures-core | 0.3.34 | Apache-2.0 OR MIT |  |
| futures-executor | 0.3.34 | Apache-2.0 OR MIT |  |
| futures-io | 0.3.34 | Apache-2.0 OR MIT |  |
| futures-lite | 2.6.1 | Apache-2.0 OR MIT | Stjepan Glavina <stjepang@gmail.com>, Contributors to futures-rs |
| futures-macro | 0.3.34 | Apache-2.0 OR MIT |  |
| futures-sink | 0.3.34 | Apache-2.0 OR MIT |  |
| futures-task | 0.3.34 | Apache-2.0 OR MIT |  |
| futures-util | 0.3.34 | Apache-2.0 OR MIT |  |
| gdk | 0.18.2 | MIT | The gtk-rs Project Developers |
| gdk-pixbuf | 0.18.5 | MIT | The gtk-rs Project Developers |
| gdk-pixbuf-sys | 0.18.0 | MIT | The gtk-rs Project Developers |
| gdk-sys | 0.18.2 | MIT | The gtk-rs Project Developers |
| gdkwayland-sys | 0.18.2 | MIT | The gtk-rs Project Developers |
| gdkx11 | 0.18.2 | MIT | The gtk-rs Project Developers |
| gdkx11-sys | 0.18.2 | MIT | The gtk-rs Project Developers |
| generic-array | 0.14.7 | MIT | Bartłomiej Kamiński <fizyk20@gmail.com>, Aaron Trent <novacrazy@gmail.com> |
| getrandom | 0.2.17 | Apache-2.0 OR MIT | The Rand Project Developers |
| getrandom | 0.3.4 | Apache-2.0 OR MIT | The Rand Project Developers |
| getrandom | 0.4.3 | Apache-2.0 OR MIT | The Rand Project Developers |
| gif | 0.14.2 | Apache-2.0 OR MIT | The image-rs Developers |
| gio | 0.18.4 | MIT | The gtk-rs Project Developers |
| gio-sys | 0.18.1 | MIT | The gtk-rs Project Developers |
| glam | 0.30.10 | Apache-2.0 OR MIT | Cameron Hart <cameron.hart@gmail.com> |
| glam | 0.31.1 | Apache-2.0 OR MIT | Cameron Hart <cameron.hart@gmail.com> |
| glam | 0.32.1 | Apache-2.0 OR MIT | Cameron Hart <cameron.hart@gmail.com> |
| glam | 0.33.7 | Apache-2.0 OR MIT |  |
| glib | 0.18.5 | MIT | The gtk-rs Project Developers |
| glib-macros | 0.18.5 | MIT | The gtk-rs Project Developers |
| glib-sys | 0.18.1 | MIT | The gtk-rs Project Developers |
| glob | 0.3.4 | Apache-2.0 OR MIT | The Rust Project Developers |
| gobject-sys | 0.18.0 | MIT | The gtk-rs Project Developers |
| gtk | 0.18.2 | MIT | The gtk-rs Project Developers |
| gtk-sys | 0.18.2 | MIT | The gtk-rs Project Developers |
| gtk3-macros | 0.18.2 | MIT | The gtk-rs Project Developers |
| half | 2.7.1 | Apache-2.0 OR MIT | Kathryn Long <squeeself@gmail.com> |
| hashbrown | 0.12.3 | Apache-2.0 OR MIT | Amanieu d'Antras <amanieu@gmail.com> |
| hashbrown | 0.16.1 | Apache-2.0 OR MIT | Amanieu d'Antras <amanieu@gmail.com> |
| hashbrown | 0.17.1 | Apache-2.0 OR MIT |  |
| hashlink | 0.12.2 | Apache-2.0 OR MIT |  |
| heck | 0.4.1 | Apache-2.0 OR MIT | Without Boats <woboats@gmail.com> |
| heck | 0.5.0 | Apache-2.0 OR MIT |  |
| hermit-abi | 0.5.3 | Apache-2.0 OR MIT |  |
| hex | 0.4.3 | Apache-2.0 OR MIT | KokaKiwi <kokakiwi@kokakiwi.net> |
| hkdf | 0.13.0 | Apache-2.0 OR MIT | RustCrypto Developers |
| hmac | 0.13.0 | Apache-2.0 OR MIT | RustCrypto Developers |
| home | 0.5.12 | Apache-2.0 OR MIT | Brian Anderson <andersrb@gmail.com> |
| html5ever | 0.38.0 | Apache-2.0 OR MIT | The html5ever Project Developers |
| http | 1.5.0 | Apache-2.0 OR MIT | Alex Crichton <alex@alexcrichton.com>, Carl Lerche <me@carllerche.com>, Sean McA |
| http-body | 1.1.0 | MIT | Carl Lerche <me@carllerche.com>, Lucio Franco <luciofranco14@gmail.com>, Sean Mc |
| http-body-util | 0.1.5 | MIT | Carl Lerche <me@carllerche.com>, Lucio Franco <luciofranco14@gmail.com>, Sean Mc |
| httparse | 1.10.1 | Apache-2.0 OR MIT | Sean McArthur <sean@seanmonstar.com> |
| hybrid-array | 0.4.15 | Apache-2.0 OR MIT | RustCrypto Developers |
| hyper | 1.11.1 | MIT | Sean McArthur <sean@seanmonstar.com> |
| hyper-rustls | 0.27.9 | Apache-2.0 OR ISC OR MIT |  |
| hyper-util | 0.1.20 | MIT | Sean McArthur <sean@seanmonstar.com> |
| iana-time-zone | 0.1.65 | Apache-2.0 OR MIT | Andrew Straw <strawman@astraw.com>, René Kijewski <rene.kijewski@fu-berlin.de>,  |
| iana-time-zone-haiku | 0.1.2 | Apache-2.0 OR MIT | René Kijewski <crates.io@k6i.de> |
| ico | 0.5.0 | MIT | Matthew D. Steele <mdsteele@alum.mit.edu> |
| icu_collections | 2.3.0 | Unicode-3.0 | The ICU4X Project Developers |
| icu_locale_core | 2.3.0 | Unicode-3.0 | The ICU4X Project Developers |
| icu_normalizer | 2.3.0 | Unicode-3.0 | The ICU4X Project Developers |
| icu_normalizer_data | 2.3.0 | Unicode-3.0 | The ICU4X Project Developers |
| icu_properties | 2.3.0 | Unicode-3.0 | The ICU4X Project Developers |
| icu_properties_data | 2.3.0 | Unicode-3.0 | The ICU4X Project Developers |
| icu_provider | 2.3.1 | Unicode-3.0 | The ICU4X Project Developers |
| ident_case | 1.0.1 | Apache-2.0 OR MIT | Ted Driggs <ted.driggs@outlook.com> |
| idna | 1.1.0 | Apache-2.0 OR MIT | The rust-url developers |
| idna_adapter | 1.2.2 | Apache-2.0 OR MIT | The rust-url developers |
| image | 0.25.10 | Apache-2.0 OR MIT | The image-rs Developers |
| image-webp | 0.2.4 | Apache-2.0 OR MIT |  |
| imageproc | 0.27.0 | MIT | theotherphil |
| imgref | 1.12.3 | Apache-2.0 OR CC0-1.0 | Kornel Lesiński <kornel@geekhood.net> |
| indexmap | 1.9.3 | Apache-2.0 OR MIT |  |
| indexmap | 2.14.2 | Apache-2.0 OR MIT |  |
| infer | 0.19.0 | MIT | Bojan <dbojan@gmail.com> |
| inout | 0.2.2 | Apache-2.0 OR MIT | RustCrypto Developers |
| interpolate_name | 0.2.4 | MIT | Luca Barbato <lu_zero@gentoo.org> |
| ipnet | 2.12.2 | Apache-2.0 OR MIT | Kris Price <kris@krisprice.nz> |
| itertools | 0.14.0 | Apache-2.0 OR MIT | bluss |
| itertools | 0.15.0 | Apache-2.0 OR MIT | bluss |
| itoa | 1.0.18 | Apache-2.0 OR MIT | David Tolnay <dtolnay@gmail.com> |
| javascriptcore-rs | 1.1.2 | MIT |  |
| javascriptcore-rs-sys | 1.1.1 | MIT | The Gtk-rs Project Developers |
| jiff | 0.2.37 | MIT OR Unlicense | Andrew Gallant <jamslam@gmail.com> |
| jiff-core | 0.1.1 | MIT OR Unlicense | Andrew Gallant <jamslam@gmail.com> |
| jiff-static | 0.2.37 | MIT OR Unlicense | Andrew Gallant <jamslam@gmail.com> |
| jiff-tzdb | 0.1.8 | MIT OR Unlicense | Andrew Gallant <jamslam@gmail.com> |
| jiff-tzdb-platform | 0.1.3 | MIT OR Unlicense | Andrew Gallant <jamslam@gmail.com> |
| jni | 0.21.1 | Apache-2.0 OR MIT | Josh Chase <josh@prevoty.com> |
| jni | 0.22.4 | Apache-2.0 OR MIT | jni team |
| jni-macros | 0.22.4 | Apache-2.0 OR MIT |  |
| jni-sys | 0.3.1 | Apache-2.0 OR MIT | Steven Fackler <sfackler@gmail.com> |
| jni-sys | 0.4.1 | Apache-2.0 OR MIT | Steven Fackler <sfackler@gmail.com>, Robert Bragg <robert@sixbynine.org> |
| jni-sys-macros | 0.4.1 | Apache-2.0 OR MIT | Robert Bragg <robert@sixbynine.org> |
| jobserver | 0.1.35 | Apache-2.0 OR MIT | Alex Crichton <alex@alexcrichton.com> |
| js-sys | 0.3.105 | Apache-2.0 OR MIT | The wasm-bindgen Developers |
| json-patch | 3.0.1 | Apache-2.0 OR MIT | Ivan Dubrov <dubrov.ivan@gmail.com> |
| jsonptr | 0.6.3 | Apache-2.0 OR MIT | chance dinkins, André Sá de Mello <codasm@pm.me> |
| keyboard-types | 0.7.0 | Apache-2.0 OR MIT | Pyfisch <pyfisch@posteo.org> |
| keyring | 4.2.0 | Apache-2.0 OR MIT | Daniel Brotsky <dev@brotsky.com> |
| keyring-core | 1.0.0 | Apache-2.0 OR MIT | Daniel Brotsky <dev@brotsky.com> |
| lazy_static | 1.5.0 | Apache-2.0 OR MIT | Marvin Löbel <loebel.marvin@gmail.com> |
| lazycell | 1.3.0 | Apache-2.0 OR MIT | Alex Crichton <alex@alexcrichton.com>, Nikita Pekin <contact@nikitapek.in> |
| lebe | 0.5.3 | BSD-3-Clause | johannesvollmer <contact@johannesvollmer.com> |
| leptonica-plumbing | 1.4.0 | MIT | Chris Couzens <ccouzens@gmail.com> |
| leptonica-sys | 0.4.9 | MIT | Chris Couzens <ccouzens@gmail.com> |
| libappindicator | 0.9.0 | Apache-2.0 OR MIT |  |
| libappindicator-sys | 0.9.0 | Apache-2.0 OR MIT |  |
| libc | 0.2.189 | Apache-2.0 OR MIT |  |
| libdbus-sys | 0.2.7 | Apache-2.0 OR MIT | David Henningsson <diwic@ubuntu.com> |
| libfuzzer-sys | 0.4.13 | (Apache-2.0 OR MIT) AND NCSA | The rust-fuzz Project Developers |
| libloading | 0.7.4 | ISC | Simonas Kazlauskas <libloading@kazlauskas.me> |
| libloading | 0.8.9 | ISC | Simonas Kazlauskas <libloading@kazlauskas.me> |
| libloading | 0.9.0 | ISC | Simonas Kazlauskas <libloading@kazlauskas.me> |
| libm | 0.2.16 | MIT | Alex Crichton <alex@alexcrichton.com>, Amanieu d'Antras <amanieu@gmail.com>, Jor |
| libredox | 0.1.24 | MIT | 4lDO2 <4lDO2@protonmail.com> |
| libsqlite3-sys | 0.38.2 | MIT | The rusqlite developers |
| linux-raw-sys | 0.12.1 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT | Dan Gohman <dev@sunfishcode.online> |
| linux-raw-sys | 0.4.15 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT | Dan Gohman <dev@sunfishcode.online> |
| litemap | 0.8.3 | Unicode-3.0 | The ICU4X Project Developers |
| litrs | 1.0.0 | Apache-2.0 OR MIT | Lukas Kalbertodt <lukas.kalbertodt@gmail.com> |
| lock_api | 0.4.14 | Apache-2.0 OR MIT | Amanieu d'Antras <amanieu@gmail.com> |
| log | 0.4.34 | Apache-2.0 OR MIT | The Rust Project Developers |
| loop9 | 0.1.5 | MIT | Kornel <kornel@geekhood.net> |
| lopdf | 0.45.0 | MIT | Junfeng Liu <china.liujunfeng@gmail.com>, Emulator <emulator@hotmail.it> |
| lru-slab | 0.1.2 | Apache-2.0 OR MIT OR Zlib | Benjamin Saunders <ben.e.saunders@gmail.com> |
| markup5ever | 0.38.0 | Apache-2.0 OR MIT | The html5ever Project Developers |
| matchers | 0.2.0 | MIT | Eliza Weisman <eliza@buoyant.io> |
| matrixmultiply | 0.3.11 | Apache-2.0 OR MIT | bluss, R. Janis Goldschmidt |
| maybe-owned | 0.3.4 | Apache-2.0 OR MIT | Philipp korber <philippkorber@gmail.com>, darkstalker <slayerbeast@gmail.com>, d |
| maybe-rayon | 0.1.1 | MIT |  |
| md-5 | 0.11.0 | Apache-2.0 OR MIT | RustCrypto Developers |
| memchr | 2.8.3 | MIT OR Unlicense | Andrew Gallant <jamslam@gmail.com>, bluss |
| memoffset | 0.9.1 | MIT | Gilad Naaman <gilad.naaman@gmail.com> |
| mime | 0.3.17 | Apache-2.0 OR MIT | Sean McArthur <sean@seanmonstar.com> |
| minimal-lexical | 0.2.1 | Apache-2.0 OR MIT | Alex Huszagh <ahuszagh@gmail.com> |
| miniz_oxide | 0.8.9 | Apache-2.0 OR MIT OR Zlib | Frommi <daniil.liferenko@gmail.com>, oyvindln <oyvindln@users.noreply.github.com |
| miniz_oxide | 0.9.1 | Apache-2.0 OR MIT OR Zlib | Frommi <daniil.liferenko@gmail.com>, oyvindln <oyvindln@users.noreply.github.com |
| mio | 1.2.3 | MIT | Carl Lerche <me@carllerche.com>, Thomas de Zeeuw <thomasdezeeuw@gmail.com>, Toki |
| moxcms | 0.8.1 | Apache-2.0 OR BSD-3-Clause | Radzivon Bartoshyk |
| muda | 0.19.3 | Apache-2.0 OR MIT |  |
| multiversion | 0.9.0 | Apache-2.0 OR MIT | Caleb Zulawski <caleb.zulawski@gmail.com> |
| multiversion-macros | 0.9.0 | Apache-2.0 OR MIT | Caleb Zulawski <caleb.zulawski@gmail.com> |
| multiversion_no_op | 1.0.0 | Apache-2.0 OR MIT | Henri Sivonen <hsivonen@hsivonen.fi> |
| nalgebra | 0.35.0 | Apache-2.0 | Sébastien Crozet <developer@crozet.re> |
| ndk | 0.9.0 | Apache-2.0 OR MIT | The Rust Mobile contributors |
| ndk-sys | 0.6.0+11769913 | Apache-2.0 OR MIT | The Rust Windowing contributors |
| new_debug_unreachable | 1.0.6 | MIT | Matt Brubeck <mbrubeck@limpet.net>, Jonathan Reem <jonathan.reem@gmail.com> |
| no_std_io2 | 0.9.4 | Apache-2.0 OR MIT |  |
| nom | 7.1.3 | MIT | contact@geoffroycouprie.com |
| nom | 8.0.0 | MIT | contact@geoffroycouprie.com |
| noop_proc_macro | 0.3.0 | MIT | Luca Barbato <lu_zero@gentoo.org> |
| ntapi | 0.4.3 | Apache-2.0 OR MIT | MSxDOS <melcodos@gmail.com> |
| nu-ansi-term | 0.50.3 | MIT | ogham@bsago.me, Ryan Scheel (Havvy) <ryan.havvy@gmail.com>, Josh Triplett <josh@ |
| num | 0.4.3 | Apache-2.0 OR MIT | The Rust Project Developers |
| num-bigint | 0.4.8 | Apache-2.0 OR MIT | The Rust Project Developers |
| num-complex | 0.4.6 | Apache-2.0 OR MIT | The Rust Project Developers |
| num-conv | 0.2.2 | Apache-2.0 OR MIT | Jacob Pratt <jacob@jhpratt.dev> |
| num-derive | 0.4.2 | Apache-2.0 OR MIT | The Rust Project Developers |
| num-integer | 0.1.47 | Apache-2.0 OR MIT | The Rust Project Developers |
| num-iter | 0.1.46 | Apache-2.0 OR MIT | The Rust Project Developers |
| num-rational | 0.4.2 | Apache-2.0 OR MIT | The Rust Project Developers |
| num-traits | 0.2.19 | Apache-2.0 OR MIT | The Rust Project Developers |
| num_cpus | 1.17.0 | Apache-2.0 OR MIT | Sean McArthur <sean@seanmonstar.com> |
| num_enum | 0.7.6 | Apache-2.0 OR BSD-3-Clause OR MIT | Daniel Wagner-Hall <dawagner@gmail.com>, Daniel Henry-Mantilla <daniel.henry.man |
| num_enum_derive | 0.7.6 | Apache-2.0 OR BSD-3-Clause OR MIT | Daniel Wagner-Hall <dawagner@gmail.com>, Daniel Henry-Mantilla <daniel.henry.man |
| objc2 | 0.6.4 | MIT | Mads Marquart <mads@marquart.dk> |
| objc2-app-kit | 0.3.2 | Apache-2.0 OR MIT OR Zlib |  |
| objc2-cloud-kit | 0.3.2 | Apache-2.0 OR MIT OR Zlib |  |
| objc2-core-data | 0.3.2 | Apache-2.0 OR MIT OR Zlib |  |
| objc2-core-foundation | 0.3.2 | Apache-2.0 OR MIT OR Zlib |  |
| objc2-core-graphics | 0.3.2 | Apache-2.0 OR MIT OR Zlib |  |
| objc2-core-image | 0.3.2 | Apache-2.0 OR MIT OR Zlib |  |
| objc2-core-location | 0.3.2 | Apache-2.0 OR MIT OR Zlib |  |
| objc2-core-text | 0.3.2 | Apache-2.0 OR MIT OR Zlib |  |
| objc2-encode | 4.1.0 | MIT | Mads Marquart <mads@marquart.dk> |
| objc2-exception-helper | 0.1.1 | Apache-2.0 OR MIT OR Zlib | Mads Marquart <mads@marquart.dk> |
| objc2-foundation | 0.3.2 | MIT |  |
| objc2-io-kit | 0.3.2 | Apache-2.0 OR MIT OR Zlib |  |
| objc2-io-surface | 0.3.2 | Apache-2.0 OR MIT OR Zlib |  |
| objc2-open-directory | 0.3.2 | Apache-2.0 OR MIT OR Zlib |  |
| objc2-quartz-core | 0.3.2 | Apache-2.0 OR MIT OR Zlib |  |
| objc2-ui-kit | 0.3.2 | Apache-2.0 OR MIT OR Zlib |  |
| objc2-user-notifications | 0.3.2 | Apache-2.0 OR MIT OR Zlib |  |
| objc2-web-kit | 0.3.2 | Apache-2.0 OR MIT OR Zlib |  |
| ocrs | 0.13.0 | Apache-2.0 OR MIT | Robert Knight |
| once_cell | 1.21.4 | Apache-2.0 OR MIT | Aleksey Kladov <aleksey.kladov@gmail.com> |
| openssl-probe | 0.2.1 | Apache-2.0 OR MIT | Alex Crichton <alex@alexcrichton.com> |
| option-ext | 0.2.0 | MPL-2.0 | Simon Ochsenreither <simon@ochsenreither.de> |
| ordered-stream | 0.2.0 | Apache-2.0 OR MIT | Daniel De Graaf <code@danieldg.net>, Zeeshan Ali Khan <zeeshanak@gnome.org> |
| owned_ttf_parser | 0.25.1 | Apache-2.0 | Alex Butler <alexheretic@gmail.com> |
| pango | 0.18.3 | MIT | The gtk-rs Project Developers |
| pango-sys | 0.18.0 | MIT | The gtk-rs Project Developers |
| parking | 2.2.1 | Apache-2.0 OR MIT | Stjepan Glavina <stjepang@gmail.com>, The Rust Project Developers |
| parking_lot | 0.12.5 | Apache-2.0 OR MIT | Amanieu d'Antras <amanieu@gmail.com> |
| parking_lot_core | 0.9.12 | Apache-2.0 OR MIT | Amanieu d'Antras <amanieu@gmail.com> |
| paste | 1.0.15 | Apache-2.0 OR MIT | David Tolnay <dtolnay@gmail.com> |
| pastey | 0.1.1 | Apache-2.0 OR MIT | Aditya Kumar <git@adityais.dev>, David Tolnay <dtolnay@gmail.com> |
| pdfium-render | 0.9.4 | Apache-2.0 OR MIT | Alastair Carey <alastair@alastaircarey.com> |
| peeking_take_while | 0.1.2 | Apache-2.0 OR MIT | Nick Fitzgerald <fitzgen@gmail.com> |
| percent-encoding | 2.3.2 | Apache-2.0 OR MIT | The rust-url developers |
| phf | 0.13.1 | MIT | Steven Fackler <sfackler@gmail.com> |
| phf_codegen | 0.13.1 | MIT | Steven Fackler <sfackler@gmail.com> |
| phf_generator | 0.13.1 | MIT | Steven Fackler <sfackler@gmail.com> |
| phf_macros | 0.13.1 | MIT | Steven Fackler <sfackler@gmail.com> |
| phf_shared | 0.13.1 | MIT | Steven Fackler <sfackler@gmail.com> |
| pin-project-lite | 0.2.17 | Apache-2.0 OR MIT |  |
| piper | 0.2.5 | Apache-2.0 OR MIT | Stjepan Glavina <stjepang@gmail.com>, John Nunley <dev@notgull.net> |
| piston-float | 1.0.1 | MIT | bvssvni <bvssvni@gmail.com> |
| pkg-config | 0.3.34 | Apache-2.0 OR MIT | Alex Crichton <alex@alexcrichton.com> |
| plist | 1.10.1 | MIT | Ed Barnard <eabarnard@gmail.com> |
| png | 0.17.16 | Apache-2.0 OR MIT | The image-rs Developers |
| png | 0.18.1 | Apache-2.0 OR MIT | The image-rs Developers |
| polling | 3.11.0 | Apache-2.0 OR MIT | Stjepan Glavina <stjepang@gmail.com>, John Nunley <dev@notgull.net> |
| portable-atomic | 1.15.0 | Apache-2.0 OR MIT |  |
| portable-atomic-util | 0.2.8 | Apache-2.0 OR MIT |  |
| potential_utf | 0.1.6 | Unicode-3.0 | The ICU4X Project Developers |
| powerfmt | 0.2.0 | Apache-2.0 OR MIT | Jacob Pratt <jacob@jhpratt.dev> |
| ppv-lite86 | 0.2.21 | Apache-2.0 OR MIT | The CryptoCorrosion Contributors |
| precomputed-hash | 0.1.1 | MIT | Emilio Cobos Álvarez <emilio@crisal.io> |
| primal-check | 0.3.4 | Apache-2.0 OR MIT | Huon Wilson <dbau.pp@gmail.com> |
| proc-macro-crate | 1.3.1 | Apache-2.0 OR MIT | Bastian Köcher <git@kchr.de> |
| proc-macro-crate | 2.0.2 | Apache-2.0 OR MIT | Bastian Köcher <git@kchr.de> |
| proc-macro-crate | 3.5.0 | Apache-2.0 OR MIT | Bastian Köcher <git@kchr.de> |
| proc-macro-error | 1.0.4 | Apache-2.0 OR MIT | CreepySkeleton <creepy-skeleton@yandex.ru> |
| proc-macro-error-attr | 1.0.4 | Apache-2.0 OR MIT | CreepySkeleton <creepy-skeleton@yandex.ru> |
| proc-macro2 | 1.0.107 | Apache-2.0 OR MIT | David Tolnay <dtolnay@gmail.com>, Alex Crichton <alex@alexcrichton.com> |
| profiling | 1.0.18 | Apache-2.0 OR MIT | Philip Degarmo <aclysma@gmail.com> |
| profiling-procmacros | 1.0.18 | Apache-2.0 OR MIT | Philip Degarmo <aclysma@gmail.com> |
| pulp | 0.22.3 | MIT | sarah quiñones <sarah@veganb.tw> |
| pulp-wasm-simd-flag | 0.1.1 | MIT | sarah quiñones <sarah@veganb.tw> |
| pxfm | 0.1.30 | Apache-2.0 OR BSD-3-Clause | Radzivon Bartoshyk |
| qoi | 0.4.1 | Apache-2.0 OR MIT | Ivan Smirnov <rust@ivan.smirnov.ie> |
| quick-error | 2.0.1 | Apache-2.0 OR MIT | Paul Colomiets <paul@colomiets.name>, Colin Kiegel <kiegel@gmx.de> |
| quick-xml | 0.41.0 | MIT |  |
| quick-xml | 0.42.0 | MIT |  |
| quinn | 0.11.11 | Apache-2.0 OR MIT |  |
| quinn-proto | 0.11.17 | Apache-2.0 OR MIT |  |
| quinn-udp | 0.5.15 | Apache-2.0 OR MIT |  |
| quote | 1.0.47 | Apache-2.0 OR MIT | David Tolnay <dtolnay@gmail.com> |
| r-efi | 5.3.0 | Apache-2.0 OR LGPL-2.1-or-later OR MIT |  |
| r-efi | 6.0.0 | Apache-2.0 OR LGPL-2.1-or-later OR MIT |  |
| rand | 0.10.2 | Apache-2.0 OR MIT | The Rand Project Developers, The Rust Project Developers |
| rand | 0.9.5 | Apache-2.0 OR MIT | The Rand Project Developers, The Rust Project Developers |
| rand_chacha | 0.9.0 | Apache-2.0 OR MIT | The Rand Project Developers, The Rust Project Developers, The CryptoCorrosion Co |
| rand_core | 0.10.1 | Apache-2.0 OR MIT | The Rand Project Developers |
| rand_core | 0.9.5 | Apache-2.0 OR MIT | The Rand Project Developers, The Rust Project Developers |
| rand_distr | 0.6.0 | Apache-2.0 OR MIT | The Rand Project Developers |
| rand_pcg | 0.10.2 | Apache-2.0 OR MIT | The Rand Project Developers |
| rangemap | 1.8.0 | Apache-2.0 OR MIT | Jeff Parsons <jeff@parsons.io> |
| rav1e | 0.8.1 | BSD-2-Clause | Thomas Daede <tdaede@xiph.org> |
| ravif | 0.13.0 | BSD-3-Clause | Kornel Lesiński <kornel@geekhood.net> |
| raw-cpuid | 11.6.0 | MIT | Gerd Zellweger <mail@gerdzellweger.com> |
| raw-window-handle | 0.6.2 | Apache-2.0 OR MIT OR Zlib | Osspial <osspial@gmail.com> |
| rawpointer | 0.2.1 | Apache-2.0 OR MIT | bluss |
| rayon | 1.12.0 | Apache-2.0 OR MIT |  |
| rayon-core | 1.13.0 | Apache-2.0 OR MIT |  |
| reborrow | 0.5.5 | MIT | sarah <> |
| redox_syscall | 0.5.18 | MIT | Jeremy Soller <jackpot51@gmail.com> |
| redox_users | 0.5.2 | MIT | Jose Narvaez <goyox86@gmail.com>, Wesley Hershberger <mggmugginsmc@gmail.com> |
| ref-cast | 1.0.27 | Apache-2.0 OR MIT | David Tolnay <dtolnay@gmail.com> |
| ref-cast-impl | 1.0.27 | Apache-2.0 OR MIT | David Tolnay <dtolnay@gmail.com> |
| regex | 1.13.1 | Apache-2.0 OR MIT | The Rust Project Developers, Andrew Gallant <jamslam@gmail.com> |
| regex-automata | 0.4.18 | Apache-2.0 OR MIT | The Rust Project Developers, Andrew Gallant <jamslam@gmail.com> |
| regex-syntax | 0.8.11 | Apache-2.0 OR MIT | The Rust Project Developers, Andrew Gallant <jamslam@gmail.com> |
| reqwest | 0.13.5 | Apache-2.0 OR MIT | Sean McArthur <sean@seanmonstar.com> |
| rfd | 0.16.0 | MIT | Poly <marynczak.bartlomiej@gmail.com> |
| rgb | 0.8.53 | MIT | Kornel Lesiński <kornel@geekhood.net>, James Forster <james.forsterer@gmail.com> |
| ring | 0.17.14 | Apache-2.0 AND ISC |  |
| roxmltree | 0.21.1 | Apache-2.0 OR MIT | Yevhenii Reizner <razrfalcon@gmail.com> |
| rsqlite-vfs | 0.1.1 | MIT |  |
| rten | 0.26.0 | Apache-2.0 OR MIT | Robert Knight |
| rten-base | 0.26.0 | Apache-2.0 OR MIT | Robert Knight |
| rten-gemm | 0.26.0 | Apache-2.0 OR MIT | Robert Knight |
| rten-imageproc | 0.26.0 | Apache-2.0 OR MIT | Robert Knight |
| rten-model-file | 0.26.0 | Apache-2.0 OR MIT | Robert Knight |
| rten-onnx | 0.26.0 | Apache-2.0 OR MIT | Robert Knight |
| rten-parallel | 0.26.0 | Apache-2.0 OR MIT | Robert Knight |
| rten-shape-inference | 0.26.0 | Apache-2.0 OR MIT | Robert Knight |
| rten-simd | 0.26.0 | Apache-2.0 OR MIT | Robert Knight |
| rten-tensor | 0.26.0 | Apache-2.0 OR MIT | Robert Knight |
| rten-vecmath | 0.26.0 | Apache-2.0 OR MIT | Robert Knight |
| rusqlite | 0.40.2 | MIT | The rusqlite developers |
| rusqlite_migration | 2.6.0 | Apache-2.0 | Clément Joly <foss@131719.xyz> |
| rustc-hash | 1.1.0 | Apache-2.0 OR MIT | The Rust Project Developers |
| rustc-hash | 2.1.3 | Apache-2.0 OR MIT | The Rust Project Developers |
| rustc_version | 0.4.1 | Apache-2.0 OR MIT |  |
| rustdct | 0.7.1 | Apache-2.0 OR MIT | Elliott Mahler <join.together at gmail> |
| rustfft | 6.4.1 | Apache-2.0 OR MIT | Allen Welkie <allen.welkie at gmail>, Elliott Mahler <join.together at gmail> |
| rustix | 0.38.44 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT | Dan Gohman <dev@sunfishcode.online>, Jakub Konka <kubkon@jakubkonka.com> |
| rustix | 1.1.4 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT | Dan Gohman <dev@sunfishcode.online>, Jakub Konka <kubkon@jakubkonka.com> |
| rustls | 0.23.44 | Apache-2.0 OR ISC OR MIT |  |
| rustls-native-certs | 0.8.4 | Apache-2.0 OR ISC OR MIT |  |
| rustls-pki-types | 1.15.1 | Apache-2.0 OR MIT |  |
| rustls-platform-verifier | 0.7.0 | Apache-2.0 OR MIT |  |
| rustls-platform-verifier-android | 0.1.1 | Apache-2.0 OR MIT |  |
| rustls-webpki | 0.103.15 | ISC |  |
| rustversion | 1.0.23 | Apache-2.0 OR MIT | David Tolnay <dtolnay@gmail.com> |
| ryu | 1.0.23 | Apache-2.0 OR BSL-1.0 | David Tolnay <dtolnay@gmail.com> |
| safe_arch | 1.2.0 | Apache-2.0 OR MIT OR Zlib | Lokathor <zefria@gmail.com> |
| same-file | 1.0.6 | MIT OR Unlicense | Andrew Gallant <jamslam@gmail.com> |
| sbwb-ai | 0.1.0 | MIT |  |
| sbwb-app | 0.1.0 | MIT |  |
| sbwb-bench | 0.1.0 | MIT |  |
| sbwb-core | 0.1.0 | MIT |  |
| sbwb-export | 0.1.0 | MIT |  |
| sbwb-image | 0.1.0 | MIT |  |
| sbwb-layout | 0.1.0 | MIT |  |
| sbwb-ocr | 0.1.0 | MIT |  |
| sbwb-pdf | 0.1.0 | MIT |  |
| sbwb-pipeline | 0.1.0 | MIT |  |
| sbwb-review | 0.1.0 | MIT |  |
| sbwb-store | 0.1.0 | MIT |  |
| sbwb-text | 0.1.0 | MIT |  |
| sbwb-worker | 0.1.0 | MIT |  |
| schannel | 0.1.29 | MIT | Steven Fackler <sfackler@gmail.com>, Steffen Butzer <steffen.butzer@outlook.com> |
| schemars | 0.8.22 | MIT | Graham Esau <gesau@hotmail.co.uk> |
| schemars | 0.9.0 | MIT | Graham Esau <gesau@hotmail.co.uk> |
| schemars | 1.2.2 | MIT | Graham Esau <gesau@hotmail.co.uk> |
| schemars_derive | 0.8.22 | MIT | Graham Esau <gesau@hotmail.co.uk> |
| scopeguard | 1.2.0 | Apache-2.0 OR MIT | bluss |
| secret-service | 5.2.0 | Apache-2.0 OR MIT | Walther Chen <walther.chen@gmail.com> |
| security-framework | 3.7.0 | Apache-2.0 OR MIT | Steven Fackler <sfackler@gmail.com>, Kornel <kornel@geekhood.net> |
| security-framework-sys | 2.17.0 | Apache-2.0 OR MIT | Steven Fackler <sfackler@gmail.com>, Kornel <kornel@geekhood.net> |
| selectors | 0.36.1 | MPL-2.0 | The Servo Project Developers |
| semver | 1.0.28 | Apache-2.0 OR MIT | David Tolnay <dtolnay@gmail.com> |
| serde | 1.0.229 | Apache-2.0 OR MIT | Erick Tryzelaar <erick.tryzelaar@gmail.com>, David Tolnay <dtolnay@gmail.com> |
| serde-untagged | 0.1.9 | Apache-2.0 OR MIT | David Tolnay <dtolnay@gmail.com> |
| serde_core | 1.0.229 | Apache-2.0 OR MIT | Erick Tryzelaar <erick.tryzelaar@gmail.com>, David Tolnay <dtolnay@gmail.com> |
| serde_derive | 1.0.229 | Apache-2.0 OR MIT | Erick Tryzelaar <erick.tryzelaar@gmail.com>, David Tolnay <dtolnay@gmail.com> |
| serde_derive_internals | 0.29.1 | Apache-2.0 OR MIT | Erick Tryzelaar <erick.tryzelaar@gmail.com>, David Tolnay <dtolnay@gmail.com> |
| serde_json | 1.0.151 | Apache-2.0 OR MIT | Erick Tryzelaar <erick.tryzelaar@gmail.com>, David Tolnay <dtolnay@gmail.com> |
| serde_repr | 0.1.21 | Apache-2.0 OR MIT | David Tolnay <dtolnay@gmail.com> |
| serde_spanned | 0.6.9 | Apache-2.0 OR MIT |  |
| serde_spanned | 1.1.1 | Apache-2.0 OR MIT |  |
| serde_urlencoded | 0.7.1 | Apache-2.0 OR MIT | Anthony Ramine <n.oxyde@gmail.com> |
| serde_with | 3.23.0 | Apache-2.0 OR MIT | Jonas Bushart, Marcin Kaźmierczak |
| serde_with_macros | 3.23.0 | Apache-2.0 OR MIT | Jonas Bushart |
| serialize-to-javascript | 0.1.2 | Apache-2.0 OR MIT | Chip Reed <chip@chip.sh> |
| serialize-to-javascript-impl | 0.1.2 | Apache-2.0 OR MIT | Chip Reed <chip@chip.sh> |
| servo_arc | 0.4.3 | Apache-2.0 OR MIT | The Servo Project Developers |
| sha2 | 0.10.9 | Apache-2.0 OR MIT | RustCrypto Developers |
| sha2 | 0.11.0 | Apache-2.0 OR MIT | RustCrypto Developers |
| sharded-slab | 0.1.7 | MIT | Eliza Weisman <eliza@buoyant.io> |
| shlex | 1.3.0 | Apache-2.0 OR MIT | comex <comexk@gmail.com>, Fenhl <fenhl@fenhl.net>, Adrian Taylor <adetaylor@chro |
| shlex | 2.0.1 | Apache-2.0 OR MIT | comex <comexk@gmail.com>, Fenhl <fenhl@fenhl.net>, Adrian Taylor <adetaylor@chro |
| signal-hook-registry | 1.4.8 | Apache-2.0 OR MIT | Michal 'vorner' Vaner <vorner@vorner.cz>, Masaki Hara <ackie.h.gmai@gmail.com> |
| simba | 0.10.2 | Apache-2.0 | sebcrozet <developer@crozet.re> |
| simd-adler32 | 0.3.10 | MIT | Marvin Countryman <me@maar.vin> |
| simd_cesu8 | 1.2.0 | Apache-2.0 OR MIT | Sean C. Roach <me@seancroach.dev> |
| simd_helpers | 0.1.0 | MIT | Luca Barbato <lu_zero@gentoo.org> |
| simdutf8 | 0.1.5 | Apache-2.0 OR MIT | Hans Kratz <hans@appfour.com> |
| siphasher | 1.0.3 | Apache-2.0 OR MIT | Frank Denis <github@pureftpd.org> |
| slab | 0.4.12 | MIT | Carl Lerche <me@carllerche.com> |
| smallvec | 1.16.1 | Apache-2.0 OR MIT | The Servo Project Developers |
| socket2 | 0.6.5 | Apache-2.0 OR MIT | Alex Crichton <alex@alexcrichton.com>, Thomas de Zeeuw <thomasdezeeuw@gmail.com> |
| softbuffer | 0.4.8 | Apache-2.0 OR MIT |  |
| soup3 | 0.5.0 | MIT |  |
| soup3-sys | 0.5.0 | MIT | The Gtk-rs Project Developers |
| spellbook | 0.4.2 | MPL-2.0 | Michael Davis <mcarsondavis@gmail.com> |
| sqlite-wasm-rs | 0.5.5 | MIT | Spxg <unsafe@outlook.es> |
| stable_deref_trait | 1.2.1 | Apache-2.0 OR MIT | Robert Grosse <n210241048576@gmail.com> |
| strength_reduce | 0.2.4 | Apache-2.0 OR MIT | Elliott Mahler <join.together@gmail.com> |
| string_cache | 0.9.0 | Apache-2.0 OR MIT | The Servo Project Developers |
| string_cache_codegen | 0.6.1 | Apache-2.0 OR MIT | The Servo Project Developers |
| stringprep | 0.1.5 | Apache-2.0 OR MIT | Steven Fackler <sfackler@gmail.com> |
| strsim | 0.11.1 | MIT | Danny Guo <danny@dannyguo.com>, maxbachmann <oss@maxbachmann.de> |
| subtle | 2.6.1 | BSD-3-Clause | Isis Lovecruft <isis@patternsinthevoid.net>, Henry de Valence <hdevalence@hdeval |
| swift-rs | 1.0.8 | Apache-2.0 OR MIT | The swift-rs contributors |
| symlink | 0.1.0 | Apache-2.0 OR MIT | Chris Morgan <me@chrismorgan.info> |
| syn | 1.0.109 | Apache-2.0 OR MIT | David Tolnay <dtolnay@gmail.com> |
| syn | 2.0.119 | Apache-2.0 OR MIT | David Tolnay <dtolnay@gmail.com> |
| syn | 3.0.5 | Apache-2.0 OR MIT | David Tolnay <dtolnay@gmail.com> |
| sync_wrapper | 1.0.2 | Apache-2.0 | Actyx AG <developer@actyx.io> |
| synstructure | 0.13.2 | MIT | Nika Layzell <nika@thelayzells.com> |
| sysinfo | 0.39.6 | MIT | Guillaume Gomez <guillaume1.gomez@gmail.com> |
| system-deps | 6.2.2 | Apache-2.0 OR MIT | Guillaume Desmottes <guillaume.desmottes@collabora.com>, Josh Triplett <josh@jos |
| tao | 0.35.3 | Apache-2.0 | Tauri Programme within The Commons Conservancy, The winit contributors |
| tao-macros | 0.1.4 | Apache-2.0 OR MIT | Tauri Programme within The Commons Conservancy |
| target-lexicon | 0.12.16 | Apache-2.0 WITH LLVM-exception | Dan Gohman <sunfish@mozilla.com> |
| tauri | 2.11.5 | Apache-2.0 OR MIT | Tauri Programme within The Commons Conservancy |
| tauri-build | 2.6.3 | Apache-2.0 OR MIT | Tauri Programme within The Commons Conservancy |
| tauri-codegen | 2.6.3 | Apache-2.0 OR MIT | Tauri Programme within The Commons Conservancy |
| tauri-macros | 2.6.3 | Apache-2.0 OR MIT | Tauri Programme within The Commons Conservancy |
| tauri-plugin | 2.6.3 | Apache-2.0 OR MIT | Tauri Programme within The Commons Conservancy |
| tauri-plugin-dialog | 2.7.3 | Apache-2.0 OR MIT | Tauri Programme within The Commons Conservancy |
| tauri-plugin-fs | 2.5.2 | Apache-2.0 OR MIT | Tauri Programme within The Commons Conservancy |
| tauri-plugin-single-instance | 2.4.4 | Apache-2.0 OR MIT | Tauri Programme within The Commons Conservancy |
| tauri-plugin-store | 2.4.4 | Apache-2.0 OR MIT | Tauri Programme within The Commons Conservancy |
| tauri-plugin-window-state | 2.4.1 | Apache-2.0 OR MIT | Tauri Programme within The Commons Conservancy |
| tauri-runtime | 2.11.3 | Apache-2.0 OR MIT | Tauri Programme within The Commons Conservancy |
| tauri-runtime-wry | 2.11.4 | Apache-2.0 OR MIT | Tauri Programme within The Commons Conservancy |
| tauri-utils | 2.9.3 | Apache-2.0 OR MIT | Tauri Programme within The Commons Conservancy |
| tauri-winres | 0.3.6 | MIT | Tauri Programme within The Commons Conservancy, Max Resch <resch.max@gmail.com> |
| tempfile | 3.27.0 | Apache-2.0 OR MIT | Steven Allen <steven@stebalien.com>, The Rust Project Developers, Ashley Mannix  |
| tendril | 0.5.1 | Apache-2.0 OR MIT | Keegan McAllister <mcallister.keegan@gmail.com>, Simon Sapin <simon.sapin@exyr.o |
| tesseract | 0.15.2 | MIT | Kevin Kwok <antimatter15@gmail.com>, Chris Couzens <ccouzens@gmail.com> |
| tesseract-plumbing | 0.11.1 | MIT | Chris Couzens <ccouzens@gmail.com> |
| tesseract-sys | 0.6.3 | MIT | Kevin Kwok <antimatter15@gmail.com>, Chris Couzens <ccouzens@gmail.com> |
| thiserror | 1.0.69 | Apache-2.0 OR MIT | David Tolnay <dtolnay@gmail.com> |
| thiserror | 2.0.20 | Apache-2.0 OR MIT | David Tolnay <dtolnay@gmail.com> |
| thiserror-impl | 1.0.69 | Apache-2.0 OR MIT | David Tolnay <dtolnay@gmail.com> |
| thiserror-impl | 2.0.20 | Apache-2.0 OR MIT | David Tolnay <dtolnay@gmail.com> |
| thread_local | 1.1.10 | Apache-2.0 OR MIT | Amanieu d'Antras <amanieu@gmail.com> |
| tiff | 0.11.3 | MIT | The image-rs Developers |
| time | 0.3.55 | Apache-2.0 OR MIT | Jacob Pratt <open-source@jhpratt.dev>, Time contributors |
| time-core | 0.1.9 | Apache-2.0 OR MIT | Jacob Pratt <open-source@jhpratt.dev>, Time contributors |
| time-macros | 0.2.32 | Apache-2.0 OR MIT | Jacob Pratt <open-source@jhpratt.dev>, Time contributors |
| tinystr | 0.8.4 | Unicode-3.0 | The ICU4X Project Developers |
| tinyvec | 1.13.2 | Apache-2.0 OR MIT OR Zlib | Lokathor <zefria@gmail.com> |
| tinyvec_macros | 0.1.1 | Apache-2.0 OR MIT OR Zlib | Soveu <marx.tomasz@gmail.com> |
| tokio | 1.53.1 | MIT | Tokio Contributors <team@tokio.rs> |
| tokio-macros | 2.7.2 | MIT | Tokio Contributors <team@tokio.rs> |
| tokio-rustls | 0.26.5 | Apache-2.0 OR MIT |  |
| tokio-util | 0.7.19 | MIT | Tokio Contributors <team@tokio.rs> |
| toml | 0.8.2 | Apache-2.0 OR MIT | Alex Crichton <alex@alexcrichton.com> |
| toml | 0.9.12+spec-1.1.0 | Apache-2.0 OR MIT |  |
| toml | 1.1.6+spec-1.1.0 | Apache-2.0 OR MIT |  |
| toml_datetime | 0.6.3 | Apache-2.0 OR MIT | Alex Crichton <alex@alexcrichton.com> |
| toml_datetime | 0.7.5+spec-1.1.0 | Apache-2.0 OR MIT |  |
| toml_datetime | 1.1.1+spec-1.1.0 | Apache-2.0 OR MIT |  |
| toml_edit | 0.19.15 | Apache-2.0 OR MIT | Andronik Ordian <write@reusable.software>, Ed Page <eopage@gmail.com> |
| toml_edit | 0.20.2 | Apache-2.0 OR MIT | Andronik Ordian <write@reusable.software>, Ed Page <eopage@gmail.com> |
| toml_edit | 0.25.15+spec-1.1.0 | Apache-2.0 OR MIT |  |
| toml_parser | 1.1.3+spec-1.1.0 | Apache-2.0 OR MIT |  |
| toml_writer | 1.1.2+spec-1.1.0 | Apache-2.0 OR MIT |  |
| tower | 0.5.3 | MIT | Tower Maintainers <team@tower-rs.com> |
| tower-http | 0.6.11 | MIT | Tower Maintainers <team@tower-rs.com> |
| tower-layer | 0.3.3 | MIT | Tower Maintainers <team@tower-rs.com> |
| tower-service | 0.3.3 | MIT | Tower Maintainers <team@tower-rs.com> |
| tracing | 0.1.44 | MIT | Eliza Weisman <eliza@buoyant.io>, Tokio Contributors <team@tokio.rs> |
| tracing-appender | 0.2.5 | MIT | Zeki Sherif <zekshi@amazon.com>, Tokio Contributors <team@tokio.rs> |
| tracing-attributes | 0.1.31 | MIT | Tokio Contributors <team@tokio.rs>, Eliza Weisman <eliza@buoyant.io>, David Bars |
| tracing-core | 0.1.36 | MIT | Tokio Contributors <team@tokio.rs> |
| tracing-log | 0.2.0 | MIT | Tokio Contributors <team@tokio.rs> |
| tracing-subscriber | 0.3.23 | MIT | Eliza Weisman <eliza@buoyant.io>, David Barsky <me@davidbarsky.com>, Tokio Contr |
| transpose | 0.2.3 | Apache-2.0 OR MIT | Elliott Mahler <join.together@gmail.com> |
| tray-icon | 0.24.2 | Apache-2.0 OR MIT |  |
| try-lock | 0.2.5 | MIT | Sean McArthur <sean@seanmonstar.com> |
| ttf-parser | 0.25.1 | Apache-2.0 OR MIT | Caleb Maclennan <caleb@alerque.com>, Laurenz Stampfl <laurenz.stampfl@gmail.com> |
| typed-path | 0.12.3 | Apache-2.0 OR MIT | Chip Senkbeil <chip@senkbeil.org> |
| typeid | 1.0.3 | Apache-2.0 OR MIT | David Tolnay <dtolnay@gmail.com> |
| typenum | 1.20.1 | Apache-2.0 OR MIT |  |
| uds_windows | 1.2.1 | MIT | Azure IoT Edge Devs, Harald Hoyer <harald@redhat.com> |
| unic-char-property | 0.9.0 | Apache-2.0 OR MIT | The UNIC Project Developers |
| unic-char-range | 0.9.0 | Apache-2.0 OR MIT | The UNIC Project Developers |
| unic-common | 0.9.0 | Apache-2.0 OR MIT | The UNIC Project Developers |
| unic-ucd-ident | 0.9.0 | Apache-2.0 OR MIT | The UNIC Project Developers |
| unic-ucd-version | 0.9.0 | Apache-2.0 OR MIT | The UNIC Project Developers |
| unicode-bidi | 0.3.18 | Apache-2.0 OR MIT | The Servo Project Developers |
| unicode-ident | 1.0.24 | (Apache-2.0 OR MIT) AND Unicode-3.0 | David Tolnay <dtolnay@gmail.com> |
| unicode-normalization | 0.1.25 | Apache-2.0 OR MIT | kwantam <kwantam@gmail.com>, Manish Goregaokar <manishsmail@gmail.com> |
| unicode-properties | 0.1.4 | Apache-2.0 OR MIT | Charles Lew <crlf0710@gmail.com>, Manish Goregaokar <manishsmail@gmail.com> |
| unicode-segmentation | 1.13.3 | Apache-2.0 OR MIT | kwantam <kwantam@gmail.com>, Manish Goregaokar <manishsmail@gmail.com> |
| untrusted | 0.9.0 | ISC | Brian Smith <brian@briansmith.org> |
| url | 2.5.8 | Apache-2.0 OR MIT | The rust-url developers |
| urlpattern | 0.3.0 | MIT | the Deno authors, crowlKats <crowlkats@toaxl.com> |
| utf16string | 0.2.0 | Apache-2.0 OR MIT | Floris Bruynooghe <flub@devork.be>, Sentry <oss@sentry.io> |
| utf8_iter | 1.0.4 | Apache-2.0 OR MIT | Henri Sivonen <hsivonen@hsivonen.fi> |
| uuid | 1.26.1 | Apache-2.0 OR MIT | Ashley Mannix<ashleymannix@live.com.au>, Dylan DPC<dylan.dpc@gmail.com>, Hunar R |
| v_frame | 0.3.9 | BSD-2-Clause | Luca Barbato <lu_zero@gentoo.org> |
| valuable | 0.1.1 | MIT |  |
| vcpkg | 0.2.15 | Apache-2.0 OR MIT | Jim McGrath <jimmc2@gmail.com> |
| vecmath | 1.0.0 | MIT | bvssvni <bvssvni@gmail.com> |
| version-compare | 0.2.1 | MIT | Tim Visee <3a4fb3964f@sinenomine.email> |
| version_check | 0.9.5 | Apache-2.0 OR MIT | Sergio Benitez <sb@sergio.bz> |
| vswhom | 0.1.0 | MIT | nabijaczleweli <nabijaczleweli@gmail.com> |
| vswhom-sys | 0.1.3 | MIT | наб <nabijaczleweli@nabijaczleweli.xyz>, forrestsmithfb <forrest.smith@fb.com> |
| walkdir | 2.5.0 | MIT OR Unlicense | Andrew Gallant <jamslam@gmail.com> |
| want | 0.3.1 | MIT | Sean McArthur <sean@seanmonstar.com> |
| wasi | 0.11.1+wasi-snapshot-preview1 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT | The Cranelift Project Developers |
| wasip2 | 1.0.4+wasi-0.2.12 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |  |
| wasm-bindgen | 0.2.128 | Apache-2.0 OR MIT | The wasm-bindgen Developers |
| wasm-bindgen-futures | 0.4.78 | Apache-2.0 OR MIT | The wasm-bindgen Developers |
| wasm-bindgen-macro | 0.2.128 | Apache-2.0 OR MIT | The wasm-bindgen Developers |
| wasm-bindgen-macro-support | 0.2.128 | Apache-2.0 OR MIT | The wasm-bindgen Developers |
| wasm-bindgen-shared | 0.2.128 | Apache-2.0 OR MIT | The wasm-bindgen Developers |
| wasm-streams | 0.5.0 | Apache-2.0 OR MIT | Mattias Buelens <mattias@buelens.com> |
| web-sys | 0.3.105 | Apache-2.0 OR MIT | The wasm-bindgen Developers |
| web-time | 1.1.0 | Apache-2.0 OR MIT |  |
| web_atoms | 0.2.6 | Apache-2.0 OR MIT | The html5ever Project Developers |
| webkit2gtk | 2.0.2 | MIT |  |
| webkit2gtk-sys | 2.0.2 | MIT |  |
| webpki-root-certs | 1.0.9 | CDLA-Permissive-2.0 |  |
| webview2-com | 0.38.2 | MIT |  |
| webview2-com-macros | 0.8.1 | MIT |  |
| webview2-com-sys | 0.38.2 | MIT |  |
| weezl | 0.1.12 | Apache-2.0 OR MIT | The image-rs Developers |
| weezl | 0.2.1 | Apache-2.0 OR MIT | The image-rs Developers |
| which | 4.4.2 | MIT | Harry Fei <tiziyuanfang@gmail.com> |
| wide | 1.7.0 | Apache-2.0 OR MIT OR Zlib | Lokathor <zefria@gmail.com> |
| winapi | 0.3.9 | Apache-2.0 OR MIT | Peter Atashian <retep998@gmail.com> |
| winapi-i686-pc-windows-gnu | 0.4.0 | Apache-2.0 OR MIT | Peter Atashian <retep998@gmail.com> |
| winapi-util | 0.1.11 | MIT OR Unlicense | Andrew Gallant <jamslam@gmail.com> |
| winapi-x86_64-pc-windows-gnu | 0.4.0 | Apache-2.0 OR MIT | Peter Atashian <retep998@gmail.com> |
| window-vibrancy | 0.6.0 | Apache-2.0 OR MIT | Tauri Programme within The Commons Conservancy |
| windows | 0.61.3 | Apache-2.0 OR MIT | Microsoft |
| windows | 0.62.2 | Apache-2.0 OR MIT |  |
| windows-collections | 0.2.0 | Apache-2.0 OR MIT |  |
| windows-collections | 0.3.2 | Apache-2.0 OR MIT |  |
| windows-core | 0.61.2 | Apache-2.0 OR MIT | Microsoft |
| windows-core | 0.62.2 | Apache-2.0 OR MIT |  |
| windows-future | 0.2.1 | Apache-2.0 OR MIT |  |
| windows-future | 0.3.2 | Apache-2.0 OR MIT |  |
| windows-implement | 0.60.2 | Apache-2.0 OR MIT |  |
| windows-interface | 0.59.3 | Apache-2.0 OR MIT |  |
| windows-link | 0.1.3 | Apache-2.0 OR MIT | Microsoft |
| windows-link | 0.2.1 | Apache-2.0 OR MIT |  |
| windows-native-keyring-store | 1.1.0 | Apache-2.0 OR MIT | Daniel Brotsky <dev@brotsky.com> |
| windows-numerics | 0.2.0 | Apache-2.0 OR MIT |  |
| windows-numerics | 0.3.1 | Apache-2.0 OR MIT |  |
| windows-result | 0.3.4 | Apache-2.0 OR MIT | Microsoft |
| windows-result | 0.4.1 | Apache-2.0 OR MIT |  |
| windows-strings | 0.4.2 | Apache-2.0 OR MIT | Microsoft |
| windows-strings | 0.5.1 | Apache-2.0 OR MIT |  |
| windows-sys | 0.45.0 | Apache-2.0 OR MIT | Microsoft |
| windows-sys | 0.52.0 | Apache-2.0 OR MIT | Microsoft |
| windows-sys | 0.59.0 | Apache-2.0 OR MIT | Microsoft |
| windows-sys | 0.60.2 | Apache-2.0 OR MIT | Microsoft |
| windows-sys | 0.61.2 | Apache-2.0 OR MIT |  |
| windows-targets | 0.42.2 | Apache-2.0 OR MIT | Microsoft |
| windows-targets | 0.52.6 | Apache-2.0 OR MIT | Microsoft |
| windows-targets | 0.53.5 | Apache-2.0 OR MIT |  |
| windows-threading | 0.1.0 | Apache-2.0 OR MIT | Microsoft |
| windows-threading | 0.2.1 | Apache-2.0 OR MIT |  |
| windows-version | 0.1.7 | Apache-2.0 OR MIT |  |
| windows_aarch64_gnullvm | 0.42.2 | Apache-2.0 OR MIT | Microsoft |
| windows_aarch64_gnullvm | 0.52.6 | Apache-2.0 OR MIT | Microsoft |
| windows_aarch64_gnullvm | 0.53.1 | Apache-2.0 OR MIT |  |
| windows_aarch64_msvc | 0.42.2 | Apache-2.0 OR MIT | Microsoft |
| windows_aarch64_msvc | 0.52.6 | Apache-2.0 OR MIT | Microsoft |
| windows_aarch64_msvc | 0.53.1 | Apache-2.0 OR MIT |  |
| windows_i686_gnu | 0.42.2 | Apache-2.0 OR MIT | Microsoft |
| windows_i686_gnu | 0.52.6 | Apache-2.0 OR MIT | Microsoft |
| windows_i686_gnu | 0.53.1 | Apache-2.0 OR MIT |  |
| windows_i686_gnullvm | 0.52.6 | Apache-2.0 OR MIT | Microsoft |
| windows_i686_gnullvm | 0.53.1 | Apache-2.0 OR MIT |  |
| windows_i686_msvc | 0.42.2 | Apache-2.0 OR MIT | Microsoft |
| windows_i686_msvc | 0.52.6 | Apache-2.0 OR MIT | Microsoft |
| windows_i686_msvc | 0.53.1 | Apache-2.0 OR MIT |  |
| windows_x86_64_gnu | 0.42.2 | Apache-2.0 OR MIT | Microsoft |
| windows_x86_64_gnu | 0.52.6 | Apache-2.0 OR MIT | Microsoft |
| windows_x86_64_gnu | 0.53.1 | Apache-2.0 OR MIT |  |
| windows_x86_64_gnullvm | 0.42.2 | Apache-2.0 OR MIT | Microsoft |
| windows_x86_64_gnullvm | 0.52.6 | Apache-2.0 OR MIT | Microsoft |
| windows_x86_64_gnullvm | 0.53.1 | Apache-2.0 OR MIT |  |
| windows_x86_64_msvc | 0.42.2 | Apache-2.0 OR MIT | Microsoft |
| windows_x86_64_msvc | 0.52.6 | Apache-2.0 OR MIT | Microsoft |
| windows_x86_64_msvc | 0.53.1 | Apache-2.0 OR MIT |  |
| winnow | 0.5.40 | MIT |  |
| winnow | 0.7.15 | MIT |  |
| winnow | 1.0.4 | MIT |  |
| winreg | 0.55.0 | MIT | Igor Shaula <gentoo90@gmail.com> |
| wit-bindgen | 0.57.1 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT | Alex Crichton <alex@alexcrichton.com> |
| writeable | 0.6.4 | Unicode-3.0 | The ICU4X Project Developers |
| wry | 0.55.1 | Apache-2.0 OR MIT | Tauri Programme within The Commons Conservancy |
| x11 | 2.21.0 | MIT | daggerbot <daggerbot@gmail.com>, Erle Pereira <erle@erlepereira.com>, AltF02 <co |
| x11-dl | 2.21.0 | MIT | daggerbot <daggerbot@gmail.com>, Erle Pereira <erle@erlepereira.com>, AltF02 <co |
| y4m | 0.8.0 | MIT | Kagami Hiiragi <kagami@genshiken.org> |
| yoke | 0.8.3 | Unicode-3.0 | Manish Goregaokar <manishsmail@gmail.com> |
| yoke-derive | 0.8.2 | Unicode-3.0 | Manish Goregaokar <manishsmail@gmail.com> |
| zbus | 5.19.0 | MIT | Zeeshan Ali Khan <zeeshanak@gnome.org> |
| zbus-secret-service-keyring-store | 1.0.1 | Apache-2.0 OR MIT | Daniel Brotsky <dev@brotsky.com> |
| zbus_macros | 5.19.0 | MIT | Marc-André Lureau <marcandre.lureau@redhat.com>, Zeeshan Ali Khan <zeeshanak@gno |
| zbus_names | 4.3.4 | MIT | Zeeshan Ali Khan <zeeshanak@gnome.org> |
| zcheapstr | 1.1.0 | MIT | Zeeshan Ali Khan <zeeshanak@gnome.org> |
| zerocopy | 0.8.57 | Apache-2.0 OR BSD-2-Clause OR MIT |  |
| zerocopy-derive | 0.8.57 | Apache-2.0 OR BSD-2-Clause OR MIT |  |
| zerofrom | 0.1.8 | Unicode-3.0 | The ICU4X Project Developers |
| zerofrom-derive | 0.1.7 | Unicode-3.0 | Manish Goregaokar <manishsmail@gmail.com> |
| zeroize | 1.9.0 | Apache-2.0 OR MIT | The RustCrypto Project Developers |
| zerotrie | 0.2.5 | Unicode-3.0 | The ICU4X Project Developers |
| zerovec | 0.11.8 | Unicode-3.0 | The ICU4X Project Developers |
| zerovec-derive | 0.11.6 | Unicode-3.0 | Manish Goregaokar <manishsmail@gmail.com> |
| zip | 8.6.0 | MIT | Mathijs van de Nes <git@mathijs.vd-nes.nl>, Marli Frost <marli@frost.red>, Ryan  |
| zlib-rs | 0.6.7 | Zlib |  |
| zmij | 1.0.23 | MIT | David Tolnay <dtolnay@gmail.com> |
| zopfli | 0.8.3 | Apache-2.0 |  |
| zune-core | 0.5.3 | Apache-2.0 OR MIT OR Zlib |  |
| zune-inflate | 0.2.54 | Apache-2.0 OR MIT OR Zlib |  |
| zune-jpeg | 0.5.15 | Apache-2.0 OR MIT OR Zlib | caleb <etemesicaleb@gmail.com> |
| zvariant | 5.15.0 | MIT | Zeeshan Ali Khan <zeeshanak@gnome.org> |
| zvariant_derive | 5.15.0 | MIT | Zeeshan Ali Khan <zeeshanak@gnome.org> |
| zvariant_utils | 4.2.0 | MIT | Zeeshan Ali Khan <zeeshanak@gnome.org>, turbocooler <turbocooler@cocaine.ninja> |

## npm packages

| Package | Version | License |
| --- | --- | --- |
| @tanstack/svelte-virtual | 3.13.38 | MIT |
| @tauri-apps/api | 2.11.1 | Apache-2.0 OR MIT |
| @tauri-apps/plugin-dialog | 2.7.3 | MIT OR Apache-2.0 |
| @tauri-apps/plugin-store | 2.4.4 | MIT OR Apache-2.0 |
| @tauri-apps/plugin-window-state | 2.4.1 | MIT OR Apache-2.0 |
| bits-ui | 2.19.2 | MIT |
| svelte-dnd-action | 0.9.79 | MIT |

## Binaries, models, fonts

- `models/best/eng.traineddata` — https://github.com/tesseract-ocr/tessdata_best/raw/main/eng.traineddata — Apache-2.0 (tesseract-ocr/tessdata)
- `models/fast/eng.traineddata` — https://github.com/tesseract-ocr/tessdata_fast/raw/main/eng.traineddata — Apache-2.0 (tesseract-ocr/tessdata)
- `models/ocrs/text-detection.rten` — https://ocrs-models.s3-accelerate.amazonaws.com/text-detection.rten — Apache-2.0 (ocrs-models)
- `models/ocrs/text-recognition.rten` — https://ocrs-models.s3-accelerate.amazonaws.com/text-recognition.rten — Apache-2.0 (ocrs-models)
- `third_party/pdfium/bin/pdfium.dll` — bblanchon/pdfium-binaries chromium/8044 — BSD-3-Clause (PDFium) / Apache-2.0 (binaries)
- `InstrumentSans[wdth,wght].ttf` — SIL Open Font License 1.1
- `JetBrainsMono[wght].ttf` — SIL Open Font License 1.1
- `Newsreader-Italic[opsz,wght].ttf` — SIL Open Font License 1.1
- `Newsreader[opsz,wght].ttf` — SIL Open Font License 1.1
- `SourceSans3-Italic[wght].ttf` — SIL Open Font License 1.1
- `SourceSans3[wght].ttf` — SIL Open Font License 1.1
- Hunspell en_GB-large word list — SCOWL (Kevin Atkinson), MIT-like permissive license
- Webster's 1913 dictionary headwords — public domain (Project Gutenberg #29765)
